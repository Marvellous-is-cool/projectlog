...
import { Document, Packer, Paragraph, Table, TableCell, TableRow, AlignmentType, TextRun } from 'docx';
  ...
  try {
    ...
    if (format === "csv") {
      ...
      const fileName = discipline 
        ? `${discipline}-submissions-${new Date().toISOString().split("T")[0]}.csv`
    ...
    if (format === "docx") {
              ...
              rows: [
                ...
                new TableRow({
                  children: [
                    ...
                    new TableCell({
                      children: [new Paragraph({ children: [new TextRun({ text: "Matric Number", bold: true })] })],
                    }),
                    new TableCell({
                      children: [new Paragraph({ children: [new TextRun({ text: "Discipline", bold: true })] })],
                    }),
                    new TableCell({
                      children: [new Paragraph({ children: [new TextRun({ text: "Project Topic", bold: true })] })],
                    }),
                    new TableCell({
                      children: [new Paragraph({ children: [new TextRun({ text: "Date Submitted", bold: true })] })],
                    }),
                  ],
                }),
                // Data rows
                ...data.map((item, index) => 
                  new TableRow({
                    children: [
                      ...
                      new TableCell({
                        children: [new Paragraph({ children: [new TextRun({ text: item["Full Name"] })] })],
                      }),
                      new TableCell({
                        children: [new Paragraph({ children: [new TextRun({ text: item["Matric Number"] })] })],
                      }),
                      new TableCell({
                        children: [new Paragraph({ children: [new TextRun({ text: item.Discipline })] })],
                      }),
                      new TableCell({
                        children: [new Paragraph({ children: [new TextRun({ text: item["Project Topic"] })] })],
                      }),
                      new TableCell({
                        children: [new Paragraph({ children: [new TextRun({ text: item["Date Submitted"] })] })],
                      }),
                    ],
                  })
                ),
      ...
      const fileName = discipline 
        ? `${discipline}-submissions-${new Date().toISOString().split('T')[0]}.docx`
